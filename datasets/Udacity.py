...
class UdacityDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(path_image_steer)):
            ...
            with open(path_image_steer[i][1], encoding='utf-8') as file:
                content = file.read()
                content.rstrip()
            steer_use.append(float(content))
    ...
    def _get_img_info(self):
        ...
        for i in range(len(os.listdir(os.path.join(self.root_dir)))):
            ...
            for i in range(len(path_images) - 39):  # 最后必须保留40个样本
                image_steer_use_item = []
                for j in range(20):  # 输入前20帧
                    image_steer = (path_images[i + j], path_steers[i + j])
                    image_steer_use_item.append(image_steer)
                image_steer_use.append(image_steer_use_item)
            ...
            for i in range(20, len(path_images) - 19):  # 最后必须保留20个样本
                ...
                for j in range(20):  # 预测20帧
                    image_steer = (path_speeds[i + j], path_steers[i + j])
                    speed_steer_label_item.append(image_steer)
                speed_steer_label.append(speed_steer_label_item)
            img_info = [(p, idx) for p, idx in zip(image_steer_use, speed_steer_label)]
...
if __name__ == "__main__":
    ...
    test_dataset = UdacityDataset(root_dir)
